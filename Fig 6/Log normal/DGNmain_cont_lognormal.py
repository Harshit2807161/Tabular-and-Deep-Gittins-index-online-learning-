...
import DGNnn_cont_lognormal as DGNnn
import torch
import copy
import csv
...
def game():
    ...
    for episode_no in tqdm(range(episodes)):
        ...
        while (state[0]!=0 or state[1]!=0 or state[2]!=0 or state[3]!=0):
            ...
            hist1.append(0.01*M[49])
    print(0.01*M)
    
    # Specify the CSV file path
    csv_file_path = 'DGN_cont_lognormal2.csv'
    
    # Write the two lists to the CSV file side by side
    with open(csv_file_path, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(['State49'])  # Optional header row
